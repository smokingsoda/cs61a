...
class Coin:
    ...
    def worth(self):
        ...
        return self.cents + diff - 50 if diff >=50 else self.cents
...
def is_bst(t):
    ...
    def bst_max(t1):
        if t1.is_leaf():
            ...
            return max([t1.label]+[bst_max(b1) for b1 in t1.branches])
        
    def bst_min(t2):
        if t2.is_leaf():
            return t2.label
        else:
            return min([t2.label]+[bst_max(b2) for b2 in t2.branches])

    def helper(t):
        if t.is_leaf():
            return True
        elif len(t.branches) == 1:
            return all([t.label >= bst_max(t.branches[0]) or t.label < bst_min(t.branches[0])]+[helper(t.branches[0])])
        else:
            return all([t.label >= bst_max(t.branches[0])]+[t.label < bst_min(t.branches[1])]+[helper(b) for b in t.branches])
    return helper(t)
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [t.label]
    for b in t.branches:
        for index in path_yielder(b, value):
            yield [t.label] + index
    "*** YOUR CODE HERE ***"
