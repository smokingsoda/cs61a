...
def g_iter(n):
    ...
    "*** YOUR CODE HERE ***"
    index = 1
    result , nresult ,nnresult = 1 ,2, 3
    if n <= 3:
        return n
    else:
        while index < n:
            result , nresult ,nnresult = nresult, nnresult , 3 * result + 2 * nresult + nnresult
            index += 1
        return result
...
def missing_digits(n):
    ...
    if n < 10 :
        ...
        if (n % 10) != (n // 10) % 10 and (n % 10) - 1 != (n // 10) % 10: 
...
def count_change(total):
    ...
    def calculate(n,m):
        if n == 0:
            ...
            return calculate(n-m,m)+calculate(n,m//2)
    index = helper(total)
    return calculate(total,index)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: lambda x :f(f,x))(lambda h,x: 1 if x == 1 else mul(x,h(h,sub(x,1))))
