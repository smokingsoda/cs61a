...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(index, term):
        if index == n:
            if term is odd_term:
                return odd_term(index)
            else:
                return even_term(index)
        elif term is odd_term:
            return odd_term(index) + helper(index+1, even_term)
        elif term is even_term:
            return even_term(index) + helper(index+1, odd_term)
...
def mutate_reverse(link):
    ...
    def copy_list(link):
        if link.rest == Link.empty:
            ...
            copy_list(link.rest)
    copy_list(link)
    def paste_list(link, index):
        if link.rest == Link.empty and index == 0:
            link.first = lst[index]
        else:
            link.first = lst[index]
            paste_list(link.rest, index - 1)
    paste_list(link, len(lst) - 1)
