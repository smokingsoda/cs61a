def prune_min(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    #prune_min: tree ---- tree(mutated) no return
    if t.is_leaf():
        return
    elif len(t.branches) == 1:
        prune_min(t.branches[0])
    else:
        if t.branches[0].label > t.branches[1].label:#prune the max one
            t.branches.pop(0)
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx): #code is right while skeleton is wrong
        """
...
def num_splits(s, d):
    ...
    result = 0
    length = len(s)
    def helper(first, rest, index):
        nonlocal result
        if index == length:
            if abs(first - rest) <= d:
                result += 1
        else:
            helper(first + s[index], rest, index + 1)
            helper(first, rest + s[index], index + 1)
    helper(s[0], 0, 1)
    return result
