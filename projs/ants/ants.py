...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while distance <= self.max_range:
            if self.min_range <= distance <= self.max_range and mid.bees:
                if isinstance(mid, Hive):
                    return None
                return rANTdom_else_none(mid.bees)
            elif isinstance(mid, Hive) or not mid.entrance:
                return None
            else:
                distance += 1
                mid = mid.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place == None:
            for index in list(current_place.bees):
                index.reduce_armor(amount + self.damage)
        else:
            for index in list(current_place.bees):
                index.reduce_armor(amount)
        # END Problem 5
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        for index in list(self.place.bees):
            index.reduce_armor(self.damage)
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.true_queen == False:
            ...
            mid = self.place.exit
            while mid:
                if mid.ant and mid.ant.buffed == False:
                    mid.ant.buffed = True
                    mid.ant.damage *= 2
                if isinstance(mid.ant, ContainerAnt):
                    if mid.ant.contained_ant and mid.ant.contained_ant.buffed == False:
                        mid.ant.contained_ant.buffed = True
                        mid.ant.contained_ant.damage *= 2
                mid = mid.exit
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.go_back == True and self.is_scared == False:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        if isinstance(self.place.ant, ContainerAnt):
            dic[mid.ant] = distance
        for index in mid.bees:
            dic[index] = distance
        mid = mid.entrance
        distance += 1
        while mid != beehive:
