...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    """
    copy_env = env
    def find_buildins(operator):
        index = 0
        nonlocal BUILTINS
        while index < len(BUILTINS):
            if operator in BUILTINS[index][3]:
                operator = BUILTINS[index][2]
                return (True, operator)
            index += 1
        return False
    operator = expr.first
    operands = expr.rest
    result = find_buildins(operator)
    if type(operator) == int or type(operator) == float:
        raise SyntaxError
    elif result[0]:
        operator = result[1]
        return operands.map(operator)
    elif operator in copy_env:
        operator = copy_env[operator]
