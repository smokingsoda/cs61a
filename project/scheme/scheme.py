...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            ...
            operands = rest.map(lambda opr: scheme_eval(opr, env, True))
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        while mid is not None:
            if symbol in mid.bindings:
                return mid.bindings[symbol]
            mid = mid.parent
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError
        if formals != nil and vals != nil:  # I am so awesome
            vals_list = iter(vals.py_list())
            formals.map(lambda index: new_frame.define(index, next(vals_list)))
        return new_frame
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        formals = target.rest
        LP = Pair(formals, body)
        env.define(name, do_lambda_form(LP, env))
        return name
        # END PROBLEM 9
    else:
...
def complete_apply(procedure, args, env): # args are expressions
