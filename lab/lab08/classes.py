...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for index in range(len(player.deck.cards)):
            player.deck.cards[index].attack += other_card.attack
            player.deck.cards[index].defense += other_card.defense
        replica = opponent.deck.cards[:]
        count = 0
        while count < len(opponent.deck.cards):
            if opponent.deck.cards[count].attack == other_card.attack or opponent.deck.cards[count].defense == other_card.defense:
                opponent.deck.cards.pop(count)
            else:
                count += 1
